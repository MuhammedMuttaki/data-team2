...
SELECT et.EmployeeID, e.FirstName || ' ' || e.LastName AS Employee, 
...
SELECT e.EmployeeID, firstname || ' ' || lastname as Employee,
ROUND(SUM((od.unitprice - (od.unitprice * od.discount)) * od.quantity), 2) AS Sale_Amount 
from Employees AS e
...
-------------------------------------------------------------------------------------------------------------
-- Week 5: 
-- 1- find the average shipping date for each category for low sales region and compare it with the Required date of the order (using shipped date - order date)

SELECT 
	ShipCountry, 
	CategoryName, 
	AVG(JULIANDAY(RequiredDate) - JULIANDAY(OrderDate)) AS "Average Required Shipment Days", 
	AVG(JULIANDAY(ShippedDate) - JULIANDAY(OrderDate)) AS "Average Actual Shipment Days" 
FROM Orders as o 
INNER JOIN "Order Details" as od 
USING(OrderID)
INNER JOIN Products as p 
...
SELECT 
	...
	CASE WHEN SUM((od.UnitPrice - (od.UnitPrice * od.Discount)) * od.Quantity) < 20000 THEN "Low"
...
WHERE ShipCountry NOT IN (
	...
	USING(OrderID)
	GROUP BY 1
	HAVING ROUND(SUM((od.unitprice-(od.unitprice*od.discount))*od.quantity), 2) BETWEEN 20000 AND 55000
	)
GROUP BY 1
ORDER BY 2 DESC